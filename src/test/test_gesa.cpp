...
int main(int argc, char* argv[]) {
    ...
    
    {
        ...
        
        {
            ...
            assert(get<1>(matches[0]) == 1);
            assert(get<1>(matches[1]) == 1);
            
            vector<pair<size_t, vector<uint64_t>>> walked;
            for (auto walked_match : gesa.walk_matches(get<0>(matches.front()),
                                                       get<1>(matches.front()))) {
                walked.push_back(walked_match);
            }
            for (auto walked_match : gesa.walk_matches(get<0>(matches.back()),
                                                       get<1>(matches.back()))) {
                walked.push_back(walked_match);
            }
            ...
            assert(walked[1].second == vector<uint64_t>{1});
            assert(walked[2].first == 0);
            assert(walked[2].second == vector<uint64_t>{2});
            assert(walked[3].first == 1);
            assert(walked[3].second == vector<uint64_t>{1});
            assert(walked[4].first == 1);
            assert(walked[4].second == vector<uint64_t>{3});
        }
