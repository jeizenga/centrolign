            ...
            if (std::get<2>(stack.back())) {
                ...
                if (std::get<1>(stack.back())) {
                    ...
                    for (size_t i = 0; i < order.size(); ++i) {
                        ...
                        for (auto next_id : graph.next(order[i])) {
                            int64_t min_dist_thru = dists_here.first + graph.label_size(next_id);
                            int64_t max_dist_thru = dists_here.second + graph.label_size(next_id);
