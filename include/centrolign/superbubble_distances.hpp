            ...
            if (std::get<2>(stack.back())) {
                ...
                if (std::get<1>(stack.back())) {
                    if (debug) {
                        ...
                        for (auto s : superbubbles.superbubbles_inside(std::get<0>(stack.back()))) {
                            std::cerr << ' ' << s;
                        }
                        std::cerr << '\n';
                    }
                    ...
                    auto& dists_here = chain_dists[std::get<0>(stack.back())];
                    const auto& links = superbubbles.superbubbles_inside(std::get<0>(stack.back()));
                    ...
                    if (debug) {
                        uint64_t s, e;
                        std::tie(s, e) = superbubbles.superbubble_boundaries(std::get<0>(stack.back()));
                        std::cerr << "reached postorder of snarl " << std::get<0>(stack.back()) << " with boundaries " << s << " and " << e << '\n';
                    }
                    
                    // is a superbubble, do DP over the net graph
                    
                    NetGraph netgraph(graph, superbubbles, std::get<0>(stack.back()));
                    ...
                    auto& bub_dists = superbubble_dists[std::get<0>(stack.back())];
